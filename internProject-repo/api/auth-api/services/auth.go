...
func GenerateTokensAndSetCookies(user *user2.Check, c echo.Context) error {
...
func generateAccessToken(user *user2.Check) (string, time.Time, error) {
...
func generateRefreshToken(user *user2.Check) (string, time.Time, error) {
	// Declare the expiration time of the token
...
func generateToken(user *user2.Check, expirationTime time.Time, secret []byte) (string, time.Time, error) {
...
func setUserCookie(user *user2.Check, expiration time.Time, c echo.Context) {
	cookie := new(http.Cookie)
	cookie.Name = "user"
		...
		if time.Unix(claims.ExpiresAt, 0).Sub(time.Now()) < 15*time.Minute {
			...
			if err == nil && rc != nil {
				...
				if tkn != nil && tkn.Valid {
					...
					_ = GenerateTokensAndSetCookies(&user2.Check{
